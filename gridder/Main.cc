...
void usage() {
  printf("Usage: hyper -d [AD4.1.DAT] -n NTHREADS(=max) -r [Receptor.PDBQT] -l [Ligand.PDBQT] (Optional: -q [[Ion+1](=0.001M),[Ion-1](=0.001M),[Ion+2](=0M),[Ion-2](=0M)] -p GRID_PADDING(=40A) -s GRID_SPACING(=0.375A)\n");
}


int main(int argc, char **argv) {
  string datfn, ligfn, recfn, stoken, token;
  double T = 298.;
  ...
  // Check to make sure all ligand atom types are in our parameter data set
  for(it=lig->types_set.begin(); it!=lig->types_set.end(); ++it) {
    bool found = false;
